...
export default function MapaProcesosSGC({ mostrarEncabezadoCategoria = true }) {
  ...
  /**
   * Renderiza una tarjeta de proceso/módulo única, uniforme y estandarizada para todo el mapa.
   * Mismo padding, radio, sombra, tamaño de ícono, tipografía y jerarquía.
   */
  const renderCard = ({
    key,
    titulo,
    codigo = null,
    subtitulo,
    subtituloLabel = 'Responsable:',
    iconSvg,
    colorTheme = 'gold',
    onClick,
    isPronto = false,
    linkTexto = 'Ver detalle →',
    tooltip,
  }) => {
    return (
      <div
        ...
        onClick={onClick}
        title={tooltip || `${titulo} ${codigo ? `(${codigo})` : ''}`}
      >
        ...
        <div className="sgc-proc-info">
          <div className="sgc-proc-header">
            <h4 className="sgc-proc-title">
              {titulo.toUpperCase()}{' '}
              {codigo && <span className="sgc-proc-code">({codigo})</span>}
            </h4>
            {isPronto && <span className="sgc-pronto-badge">Muy pronto</span>}
          </div>
          <p className="sgc-proc-responsable">
            <span className="sgc-resp-label">{subtituloLabel}</span>{' '}
            <span className="sgc-resp-name">{subtitulo}</span>
          </p>
          <div className="sgc-proc-footer">
            <span className="sgc-link-detalle">
              {isPronto ? '✨ ' : '👤 '}{linkTexto}
            </span>
          </div>
        </div>
  ...
  /**
   * Helper para renderizar un proceso SGC existente en BD
   */
  const renderProcesoCalidadCard = (proc, iconSvg, colorTheme = 'gold', customTitle = null) => {
    if (!proc) return null;
    const responsiblesText =
      proc.responsables && proc.responsables.length > 0
        ? proc.responsables.map((r) => r.usuario?.nombre || 'Usuario').join(', ')
        : 'Sin asignar';
    const displayName = customTitle || proc.nombre;

    return renderCard({
      key: proc.id,
      titulo: displayName,
      codigo: proc.codigo,
      subtitulo: responsiblesText,
      subtituloLabel: 'Responsable:',
      iconSvg,
      colorTheme,
      onClick: () => navigate(`/calidad-de-procesos/proceso/${proc.id}`),
      linkTexto: 'Ver detalle →',
      tooltip: `Ver detalle del proceso ${proc.nombre} (${proc.codigo})`,
    });
  };
          ...
          <div className="sgc-map-layout">
            ...
            <div className="sgc-center-blocks">
              ...
              <section className="sgc-section-block sgc-section-block--apoyo">
                ...
                <div className="sgc-cards-row sgc-cards-row--apoyo sgc-apoyo-layout-grid">
                  {/* 1. Ambiental (SA / GA) */}
                  <div className="sgc-apoyo-col">
                    <div className="sgc-subgroup-header sgc-subgroup-header--spacer">
                      <span className="sgc-subgroup-title" style={{ visibility: 'hidden' }}>Ambiental</span>
                    </div>
                    {ambiental &&
                      renderProcesoCalidadCard(
                        ambiental,
                  ...
                  {/* 2. Administrativo (AD): Grupo contenedor con Administrativo y Escuela GSB anidada */}
                  <div className="sgc-apoyo-col sgc-apoyo-col--administrativo">
                    <div className="sgc-subgroup-grid">
                      {administrativo &&
                        renderProcesoCalidadCard(
                          administrativo,
                  ...
                  <div className="sgc-apoyo-col">
                    <div className="sgc-subgroup-header sgc-subgroup-header--spacer">
                      <span className="sgc-subgroup-title" style={{ visibility: 'hidden' }}>SG - SST</span>
                    </div>
                    {sgsst &&
                      renderProcesoCalidadCard(
                        sgsst,
